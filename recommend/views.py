...
def detail(request, movie_id):
    ...
    results = YoutubeSearch(movies.title+"trailer english", max_results=1).to_json()
    link = results.partition("?v=")[2].split('"}]}')[0]
    context = {'movies': movies, 'movie_rating': movie_rating, 'rate_flag': rate_flag, 'update': update,'link':link}
...
def get_similar(movie_name, rating, corrMatrix):
    similar_ratings = corrMatrix[movie_name] * (rating - 1)
...
def recommend(request):
    ...
    movie_list = list(Movie.objects.filter(id__in=movies_id_recommend).order_by(preserved)[:20])
...
def signUp(request):
    if request.user.is_authenticated:
        ...
        context = {'form': form}
        return render(request, 'recommend/register.html', context)
...
def Login(request):
    if request.user.is_authenticated:
        ...
        context = {}
        return render(request, 'recommend/login.html', context)
...
def edit_profile(request):
    ...
    return render(request, 'recommend/edit_profile.html', {'form': form})
...
def change_pass(request):
    ...
    return render(request, 'recommend/password_change.html', {'form': fm})
